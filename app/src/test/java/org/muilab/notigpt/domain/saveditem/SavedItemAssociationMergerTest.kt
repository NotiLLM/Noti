...
/**
 * Unit tests for [SavedItemAssociationMerger.filterValidEvidence], the pure evidence-guard logic
 * used by the extraction handler when applying contract-v2 ops.
 *
 * (The JSON-reading [SavedItemAssociationMerger.evidenceIdsFrom] relies on android's org.json, which is a
 * non-functional stub under plain JVM unit tests, so it is exercised by instrumented tests instead.)
 */
class SavedItemAssociationMergerTest {

    @Test
    fun `filterValidEvidence keeps only ids present in the request`() {
        val valid = SavedItemAssociationMerger.filterValidEvidence(
